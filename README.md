...
**Selecting & copying a whole block** — three gestures grab a task *and its indented sub-block* as a unit. **Double-click** a task's marker, bullet, or indentation to select the whole block — while double-clicking the task's *text* still selects a single word (the marker is the "handle", much like double-clicking a `{` vs. a word in code); turn this off with `tsk.doubleClickSelectsBlock`. With **nothing selected**, `Ctrl+C` / `Ctrl+X` on a task line copy / cut the whole block instead of just the one line VS Code's "copy line" would take — cut removes it cleanly (no orphaned blank line), copy leaves it selected, and the clipboard gets a trailing newline so the block pastes as its own line (toggle with `tsk.copyBlockTrailingNewline`); any existing selection falls straight through to the native copy/cut. And **Tsk: Select Task Block** (Command Palette, bindable to a key of your choice) is the no-mouse equivalent. All scoped to `.tsk` editors, and the block is whatever the cursor's task owns by indentation — the same range Move/Extract relocate.
...
| `tsk.copyBlockTrailingNewline` | `true` | When `Ctrl+C` / `Ctrl+X` copies/cuts a whole task block (nothing selected), append a trailing newline so the block pastes as a standalone unit on its own line. Off ⇒ the clipboard ends at the block's last character. See *List editing*. |
