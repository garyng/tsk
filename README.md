...
- **`Alt+T` find-all-tasks-by-tag.** Opens a `QuickPick` of every known tag — each row showing a hierarchical task count (`5 tasks · <description>`), searchable by name, count, *and* yaml description via `matchOnDescription: true` — then dispatches `search.action.openNewEditor` (VSCode's **Search Editor**) with `#<tag>` pre-queried and the include glob scoped to `*.tsk`. The Search Editor is a full result tab with Ctrl+Click navigation, regex/case toggles, and result folding for free — no custom result document needed — and the search opens with **0 context lines** (M30) so only the matching task rows show. Its `search-result` grammar embeds only built-in languages, so it does *not* apply tsk's own grammar to result rows; instead the extension paints tsk **decorations** (marker colors, priority backgrounds, dimmed metadata) onto the match rows itself, offsetting each range past the `  <lineNo>: ` gutter (`computeSearchResultRanges`, M30/B). The per-tag count is prefix-inclusive (a `#project` row counts its `#project/tsk` tasks too) so it matches what the substring search returns.
