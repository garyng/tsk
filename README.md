...
**Selecting & copying a whole block** — three gestures grab a task *and its indented sub-block* as a unit. **Double-click** a task's marker, bullet, or indentation to select the whole block — while double-clicking the task's *text* still selects a single word (the marker is the "handle", much like double-clicking a `{` vs. a word in code); turn this off with `tsk.doubleClickSelectsBlock`. With **nothing selected**, `Ctrl+C` / `Ctrl+X` on a task line copy / cut the whole block instead of just the one line VS Code's "copy line" would take — cut removes it cleanly (no orphaned blank line), copy leaves it selected, and block copy / cut / select all include the trailing line break — so a block pastes as its own line (toggle with `tsk.blockTrailingNewline`); any existing selection falls straight through to the native copy/cut. And **Tsk: Select Task Block** (Command Palette, bindable to a key of your choice) is the no-mouse equivalent. All scoped to `.tsk` editors, and the block is whatever the cursor's task owns by indentation — the same range Move/Extract relocate.
...
- **Populate `tags.yml` from your tags** (Command Palette → *Tsk: Add Discovered Tags to tags.yml*). Appends every tag used on a task that isn't declared yet as a bare stub under a dated `# discovered <date>` header — **append-only**, so existing entries and comments are untouched — ready for you to fill in a `description` / `parent`. Literal tags only (implicit `/`-parents aren't auto-added); re-running adds only what's new. Creates the file if it doesn't exist.
...
- **Tsk: Open Task List** — every task in a **searchable, sortable table**: a fuzzy content search, a relative `Created` column (newest first, hover for the exact time), coloured `P1/P2/P3` priority badges (`@priority:1–3`), and **Excel-style header filter dropdowns** on Status, Tags, and Priority — faceted, multi-select, with a search box for tags. The status chips (each showing its marker glyph + live count) are one-click quick-filters over the same Status filter, and a **Current file** toggle narrows the list to the `.tsk` file you're editing — following you as you switch editors (off by default; disabled until a `.tsk` file is active). A row click reveals the task beside the panel. Virtualized for large workspaces. The persistent, browse-and-stay companion to *Tsk: Find All Tasks by Status* (which stays the fast keyboard path).
...
| `tsk.doubleClickSelectsBlock` | `true` | Double-click a task's marker / bullet / indentation in a `.tsk` editor to select its whole block; double-clicking the task's text still selects a word. Off ⇒ default word-select everywhere. See *List editing*. |
| `tsk.blockTrailingNewline` | `true` | Include the trailing line break when operating on a whole task block — `Ctrl+C` / `Ctrl+X` copy/cut (nothing selected) and double-click / `Tsk: Select Task Block` selection all extend through it, so a block is a self-contained unit on its own line. Off ⇒ block operations end at the block's last character. See *List editing*. |
